...
// ----------------------------------------------------------------------
// Agre un escuchador de evento para que cuando cambie el input  llama la funcion buscar mascota
buscador.addEventListener('input', buscarMascota)
// creo la función que me va a servir para buscar una mascota
function buscarMascota() {
  // obtengo el valor del buscador y con toLowerCase convertimos todo a minuscula  para que la busqueda no se enfoque en solo mayusculas y minusculas.
  const textoBusqueda = buscador.value.toLowerCase();  
  
  // Limpio la tabla para que solo se muestren los resultados de lo que estoy buscando
  tabla.innerHTML = "";

// Creo constante de newArray que tendra todas mascotas que cumplan con la condicion del filter.
    const newArray = arregloMascotas.filter((masc)=>masc.nombre.toLowerCase().includes(textoBusqueda))
    // Si encuentra mas de un elemento lo va mapear en la tabla.
    if (newArray.length >= 1) {
      // si es asi, se crea una nueva fila con la mascota que se encontró
      newArray.forEach((elemento)=>crearFila(elemento)) 
    //   S no, nos retorna un mensaje de error.
    } else{
        tabla.innerHTML = `
        <div>
